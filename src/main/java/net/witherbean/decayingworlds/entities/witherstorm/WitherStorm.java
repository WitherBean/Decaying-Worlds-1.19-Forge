package net.witherbean.decayingworlds.entities.witherstorm;
...
import net.witherbean.decayingworlds.entities.witherstorm.parts.WitherStormPart;

public class WitherStorm extends Monster implements PowerableMob, RangedAttackMob {
   ...
   private static final TargetingConditions TARGETING_CONDITIONS = TargetingConditions.forCombat().range(150.0D).selector(LIVING_ENTITY_SELECTOR);

   public WitherStorm(EntityType<? extends WitherStorm> p_31437_, Level p_31438_) {
   ...
   protected void registerGoals() {
      this.goalSelector.addGoal(0, new WitherStorm.WitherDoNothingGoal());
   ...
   public boolean hurt(DamageSource p_31461_, float p_31462_) {
      if (this.isInvulnerableTo(p_31461_)) {
         return false;
      } else if (p_31461_ != DamageSource.DROWN && !(p_31461_.getEntity() instanceof WitherStorm)) {
   ...
   public static AttributeSupplier.Builder createAttributes() {
      return Monster.createMonsterAttributes().add(Attributes.MAX_HEALTH, 1024.0D).add(Attributes.MOVEMENT_SPEED, (double)0.6F).add(Attributes.FLYING_SPEED, (double)0.6F).add(Attributes.FOLLOW_RANGE, 10000.0D).add(Attributes.ARMOR, 4.0D);
   }
...
public boolean hurt(WitherStormPart witherStormPart, DamageSource p_31020_, float p_31021_) {
    return false;
}
}
